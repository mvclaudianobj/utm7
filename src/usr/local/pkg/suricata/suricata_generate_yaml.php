...
if ($suricatacfg['blockoffenders'] == 'on' && ($suricatacfg['ips_mode'] == 'ips_mode_legacy' || $suricatacfg['ips_mode'] == 'ips_mode_mix'))
...
//Is FAPP
if (!in_array($if_real, $all_gtw,true)) {
	$VARIABLE_TABLE_SURICATA = "fapp2c";
}


//$suri_pf_table = SURICATA_PF_TABLE;
$suri_pf_table = $VARIABLE_TABLE_SURICATA;
...
// Redis eve or regular file eve
$all_gtw = getInterfacesInGatewaysWithNoExceptions();
if (in_array($if_real, $all_gtw, true)) {
	if (file_exists('/etc/redis_enabled')) {
		if (trim(file_get_contents('/etc/redis_enabled')) == 'on') {
			$eve_output_type = 'redis';
		} else {
...
if (($suricatacfg['eve_log_alerts'] == 'on')) {
	...
	$eve_out_types .= "\n            http-body: " . (
	    ($suricatacfg['eve_log_alerts_payload'] == 'on' || $suricatacfg['eve_log_alerts_payload'] == 'only-base64') 
	    ? 'yes' 
	    : 'no '
	) . "            # enable dumping of http body in Base64";
	...
	$eve_out_types .= "\n            metadata: ".($suricatacfg['eve_log_alerts_metadata'] == 'on'?'yes':'yes ')."             # enable inclusion of app layer metadata with alert";
...
$iprep_path = rtrim(SURICATA_IPREP_PATH, '/');
$iprep_config = "# IP Reputation\n # IP Reputation
reputation-categories-file: /usr/local/share/suricata/acp/rules/categories.txt
default-reputation-path: /usr/local/share/suricata/acp/rules/
reputation-files:
 - reputation.list
";
