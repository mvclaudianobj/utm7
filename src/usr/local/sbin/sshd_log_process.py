...
class sshd_process(Thread):
	def __init__(self, file_sshd):
		Thread.__init__(self)
		self.fd = file_sshd
		self.sshd_logs = []

	def tail(self, fd, callback, delay=.1):
		fd.seek(0, 1) # seek to the end
	...
	def init_process_data(self, line):
		self.sshd_logs.append(line)

		if len(self.sshd_logs) > 0:
			self.start_process_data_sshd_fapp = process_data_fapp(self.sshd_logs, 'sshd')
			self.start_process_data_sshd_fapp.start()
			self.sshd_logs = []
